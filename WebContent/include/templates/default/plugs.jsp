...
<aside>
	...
	<div class="widget">
		<h3>${_res.link}</h3>
		<ul>
			<c:forEach items="${init.links}" var="link"><!--${link.url }-->
				<!--<li><a href="www.icodee.com" title="${link.alt }" target="_blank">${link.linkName}</a></li>-->
				<a href="javascript:void(0)" title="${link.alt }" target="_blank" onclick="pop();">${link.linkName}</a>
				<script type="text/javascript">
					function pop(){
						//window.location.href='http://www.icodee.com';
						window.open('http://www.icodee.com')
					}
				</script>
			</c:forEach>
		</ul>
